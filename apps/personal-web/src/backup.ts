...
export async function encryptProfile(profile: TwinProfile, password: string): Promise<EncryptedBackup> {
  ...
  const encrypted = await crypto.subtle.encrypt({ name: "AES-GCM", iv: iv as unknown as BufferSource }, key, encoder.encode(JSON.stringify(profile)));
...
export async function decryptProfile(backup: EncryptedBackup, password: string): Promise<TwinProfile> {
  ...
  try {
    ...
    return twinProfileSchema.parse(JSON.parse(decoder.decode(plain)));
